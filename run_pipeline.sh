...
if [[ $# -lt 1 ]]; then
  echo "Usage: $0 <target_folder> [youtube_url] [scale]" >&2
  exit 1
fi
...
"${REPO_ROOT}/03_upscale.sh" "$frames_dir" "$upscaled_dir" "$SCALE"
...
output_name="${video_base}_realesrgan_x4plus_${SCALE}x_HQ.mkv"
